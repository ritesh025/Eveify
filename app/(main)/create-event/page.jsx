...
import { CalendarIcon, Loader2, Palette, Sparkles } from "lucide-react";
...
const eventSchema = z.object({
  ...
  description: z.string().min(22, "Description must be at least 22 characters"),
...
export default function CreateEventPage() {
  ...
  // Color presets - show all for premium, only default for Free
  const colorPresets = [
    "#0C2B4E", // Default color (always available)
    ...(hasPremium
      ? [
          "#92400e",
          "#7f1d1d",
  ...
  const handleColorClick = (color) => {
    // If not default color and user doesn't have premium
    if (color !== "#5eb2f2" && !hasPremium) {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (!hasPremium && currentUser?.freeEventsCreated >= 1) {
        setUpgradeReason("limit");
        setShowUpgradeModal(true);
        return;
      }

      // Check if trying to use custom color without premium
      if (data.themeColor !== "#1e3a8a" && !hasPremium) {
        setUpgradeReason("color");
        setShowUpgradeModal(true);
        return;
      }
          ...
          >
            {coverImage ? (
              ...
              <span className="opacity-60 text-sm">
                Click to add cover image of Event
              </span>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              {!hasPremium && (
                <button
                  ...
                  title="Unlock more colors with premium"
                >
            ...
            {!hasPremium && (
              <p className="text-xs text-muted-foreground">
                Upgrade to premium to unlock custom theme colors.
              </p>
            )}
          </div>
        </div>
